$(document).ready(function(){
  search_fetch("", "NHACUNGCAP", 1) // Khởi tạo trang đầu
})
...
function submitAdd(){
  ...
  let checkPhone = 1
    if(!/[a-zA-ZÀ-ý][a-zA-Z0-9-_ ]{4,24}/.test($('.form_data input[name = "fullName"]').val())){
      $('.form_data input[name = "fullName"] + div span').text("Tên không thuộc khoảng 4 - 24 hay không lợp lệ")
      checkName = 0
    } else {
    ...
    if(checkName === 1 && checkAddress === 1 && checkPhone === 1){
      $.ajax({
        ...
        data: {
          access: "NHACUNGCAP",
          info: {
            MaNCC: $('.form_data input[name = "code"]').val(),
...
function input_add_ncc(subject, column){
...
function input_edit_ncc(access){
  let _this = access
